...
export async function POST(request: NextRequest) {
  ...
  try {
    const { mariageId, url } = await request.json()
    const photo = await prisma.mariagePhoto.create({ data: { mariageId: parseInt(mariageId), url } })
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    ...
    await prisma.mariagePhoto.delete({ where: { id: parseInt(id) } })
