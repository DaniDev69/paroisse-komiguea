...
type Mariage = { id: number; nomEpoux: string; prenomEpoux: string; nomEpouse: string; prenomEpouse: string; categorie: string; annee: number; pereCelebrant: string; photos: Photo[] }

const CoupleCard = ({ couple }: { couple: Mariage }) => {
  const [photoIndex, setPhotoIndex] = useState(0)

  useEffect(() => {
    if (couple.photos.length <= 1) return
    const interval = setInterval(() => {
      setPhotoIndex((prev) => (prev + 1) % couple.photos.length)
    }, 2500)
  ...
  return (
    ...
    >
      <div style={{ position: 'relative', height: '220px', background: 'linear-gradient(135deg, #4A2E08 0%, #8A5A18 100%)', overflow: 'hidden' }}>
        {couple.photos.length > 0 ? (
          <>
            <Image src={couple.photos[photoIndex].url} alt={couple.prenomEpoux} fill sizes="33vw" style={{ objectFit: 'cover', transition: 'opacity 0.5s' }}/>
      ...
      <div style={{ padding: '18px 20px' }}>
        <h3 style={{ fontFamily: 'Cinzel, serif', fontSize: '14px', fontWeight: 600, color: '#0D2B55', marginBottom: '6px', lineHeight: 1.4 }}>
          {couple.prenomEpoux} {couple.nomEpoux} & {couple.prenomEpouse} {couple.nomEpouse}
        </h3>
        <p style={{ fontFamily: 'Lato, sans-serif', fontSize: '11px', color: '#C9A84C', letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '4px' }}>{couple.annee}</p>
        {couple.pereCelebrant && (
          <p style={{ fontFamily: 'Crimson Text, serif', fontSize: '14px', color: '#6B7A8D', fontStyle: 'italic' }}>Père : {couple.pereCelebrant}</p>
        )}
        <span style={{ display: 'inline-block', marginTop: '8px', background: '#FBF4E0', color: '#7A4A10', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>{couple.categorie}</span>
      </div>
    </div>
  )
}
...
export default function MariagePage() {
  ...
  useEffect(() => {
    fetch('/api/listes/mariages')
      .then(r => r.json())
      .then(data => { setMariages(Array.isArray(data) ? data : []); setChargement(false) })
      .catch(() => setChargement(false))
  }, [])

  const visibles = voirTout ? mariages : mariages.slice(0, 6)

  const handleDownloadPhotos = async () => {
    if (!anneeDownload) return alert('Veuillez sélectionner une année')
    const filtered = mariages.filter(m => m.annee === parseInt(anneeDownload))
    if (filtered.length === 0) return alert('Aucun mariage pour cette année')
    const photos = filtered.flatMap(m => m.photos.map(p => p.url))
    if (photos.length === 0) return alert('Aucune photo disponible pour cette année')
    for (let i = 0; i < photos.length; i++) {
      const link = document.createElement('a')
      link.href = photos[i]
      link.download = `mariage_${anneeDownload}_${i + 1}.jpg`
      link.click()
      ...
      <main>
        <section style={{ background: 'linear-gradient(160deg, #4A2E08 0%, #8A5A18 100%)', padding: '80px 40px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            <p style={{ fontFamily: 'Cinzel, serif', fontSize: '11px', letterSpacing: '3px', textTransform: 'uppercase', color: '#FBF4E0', marginBottom: '16px' }}>Sacrement</p>
            ...
            <p style={{ fontFamily: 'Crimson Text, serif', fontSize: '22px', color: '#FBF4E0', fontStyle: 'italic', maxWidth: '650px', margin: '0 auto 32px' }}>
              Vous êtes sur votre espace dédié au sacrement du <strong style={{ color: '#FFD700' }}>Mariage</strong>
            </p>
            <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
              <Link href="/culte/mariage/liste" style={{ display: 'inline-block', background: '#C9A84C', color: '#4A2E08', fontFamily: 'Lato, sans-serif', fontSize: '12px', fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', padding: '14px 32px', textDecoration: 'none', borderRadius: '4px' }}>
                Consulter la Liste
              </Link>
            ...
            {showDownload && (
              <div style={{ maxWidth: '400px', margin: '24px auto 0', background: 'rgba(255,255,255,0.1)', borderRadius: '10px', padding: '24px', border: '1px solid rgba(255,255,255,0.2)' }}>
                <p style={{ fontFamily: 'Cinzel, serif', fontSize: '12px', letterSpacing: '1px', color: '#C9A84C', marginBottom: '12px', textTransform: 'uppercase' }}>Choisir une Année</p>
                ...
                <button onClick={handleDownloadPhotos} style={{ width: '100%', padding: '12px', background: '#C9A84C', color: '#4A2E08', border: 'none', borderRadius: '6px', fontFamily: 'Lato, sans-serif', fontSize: '12px', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer' }}>
                  ⬇ Télécharger
                </button>
              </div>
            )}
        ...
        <section style={{ background: '#FFFFFF', padding: '60px 40px' }}>
          <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
            ...
            <h2 style={{ fontFamily: 'Cinzel, serif', fontSize: '30px', fontWeight: 400, color: '#0D2B55', marginBottom: '32px' }}>Le Mariage, Alliance d&apos;Amour</h2>
            ...
            <p style={{ fontFamily: 'Crimson Text, serif', fontSize: '19px', color: '#4A5568', lineHeight: 1.85 }}>
              Le Mariage chrétien est une alliance sacrée entre un homme et une femme, image de l&apos;amour fidèle et indissoluble de Dieu pour son peuple.
            </p>
          </div>
        </section>
        ...
        <section style={{ background: '#FAF8F3', padding: '60px 40px' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            ...
            <h2 style={{ fontFamily: 'Cinzel, serif', fontSize: '30px', fontWeight: 400, color: '#0D2B55' }}>Nos Couples Mariés</h2>
          ...
          {chargement ? (
            ...
            <div style={{ textAlign: 'center', padding: '40px' }}><p style={{ fontFamily: 'Crimson Text, serif', fontSize: '18px', color: '#6B7A8D', fontStyle: 'italic' }}>Aucun mariage enregistré pour le moment.</p></div>
          ) : (
            <>
              <div className="cards-grid">
                {visibles.map((m) => <CoupleCard key={m.id} couple={m} />)}
              </div>
              {mariages.length > 6 && (
                <div style={{ textAlign: 'center', marginTop: '40px' }}>
                  <button onClick={() => setVoirTout(!voirTout)} style={{ background: '#8A5A18', color: '#FBF4E0', border: 'none', borderRadius: '6px', padding: '14px 36px', fontFamily: 'Lato, sans-serif', fontSize: '12px', fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', cursor: 'pointer' }}>
                    {voirTout ? '▲ Voir Moins' : `▼ Voir Plus (${mariages.length - 6} restants)`}
                  </button>
                </div>
              )}
            </>
