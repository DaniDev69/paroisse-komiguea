...
export default function MariageListePage() {
...
const handleDownload = async () => {
  ...
  try {
    const res = await fetch(`/api/listes/mariage?annee=${annee}&categorie=${encodeURIComponent(categorie)}`)
    ...
    doc.setFontSize(13)
    doc.text(`Liste des mariés — ${categorie} — ${annee}`, 14, 32)
    doc.setFontSize(10)
    doc.text('Nom', 14, 50)
    ...
    doc.save(`mariage_${annee}_${categorie}.pdf`)
    ...
    link.href = url
    link.download = `mariage_${annee}_${categorie}.csv`
    link.click()
      ...
      <main>
        <section style={{ background: 'linear-gradient(160deg, #4A2E08 0%, #8A5A18 100%)', padding: '70px 40px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <h1 style={{ fontFamily: 'Crimson Text, serif', fontSize: '44px', fontStyle: 'italic', color: '#FFFFFF', lineHeight: 1.2 }}>Liste des Mariages</h1>
          </div>
        </section>
        ...
        <section style={{ background: '#FAF8F3', padding: '80px 40px' }}>
          <style>{`
            ...
            .download-btn { width: 100%; padding: 16px; background: #8A5A18; color: #FFFFFF; border: none; border-radius: 6px; font-family: Lato, sans-serif; font-size: 12px; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; cursor: pointer; transition: background 0.2s, opacity 0.2s; margin-top: 8px; }
            .download-btn:hover { background: #C9A84C; color: #4A2E08; }
          ...
          <div className="liste-form">
            <div className="form-group">
              <label className="form-label">Année de Mariage</label>
            ...
            <div style={{ background: '#FBF4E0', borderRadius: '8px', padding: '16px 20px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              <span style={{ fontSize: '20px' }}>ℹ️</span>
              <p style={{ fontFamily: 'Crimson Text, serif', fontSize: '15px', color: '#4A2E08', fontStyle: 'italic', margin: 0 }}>
                La liste contiendra : Nom & Prénom des époux, Année de mariage et Père célébrant
              </p>
            </div>
        ...
        <section style={{ background: '#FFFFFF', padding: '40px', textAlign: 'center' }}>
          <Link href="/culte/mariage" style={{ display: 'inline-block', background: '#0D2B55', color: '#FBF4E0', fontFamily: 'Lato, sans-serif', fontSize: '11px', fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', padding: '14px 32px', textDecoration: 'none', borderRadius: '3px' }}>
            ← Retour Mariage
          </Link>
        </section>
      </main>
