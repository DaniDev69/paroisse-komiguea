...
const sacrements = [
  {
    titre: 'Mariage',
    icon: '💍',
    desc: 'Célébrez votre union sacrée dans la joie et la foi. Notre paroisse vous accompagne dans la préparation et la célébration de ce beau sacrement.',
    href: '/culte/mariage',
  ...
  },
  {
    titre: 'Communion',
    icon: '🍞',
    desc: 'Préparez-vous à recevoir pour la première fois le Corps et le Sang du Christ. Un moment inoubliable dans la vie du chrétien.',
            ...
            {sacrements.map((s) => (
              <div key={s.titre} className="sacrement-card">
                <div className="sacrement-card-top" style={{ background: s.bg }}>
                  <span style={{ fontSize: '52px', position: 'relative', zIndex: 1 }}>{s.icon}</span>
                  <span style={{ fontFamily: 'Cinzel, serif', fontSize: '18px', fontWeight: 600, color: '#FFFFFF', position: 'relative', zIndex: 1 }}>{s.titre}</span>
                </div>
                <div className="sacrement-card-body">
                  <p style={{ fontFamily: 'Crimson Text, serif', fontSize: '18px', color: '#4A5568', lineHeight: 1.75, flex: 1 }}>
                    {s.desc}
                  </p>
                  <Link href={s.href} style={{ display: 'inline-block', background: s.btnColor, color: s.btnText, fontFamily: 'Lato, sans-serif', fontSize: '11px', fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', padding: '12px 24px', textDecoration: 'none', borderRadius: '4px', textAlign: 'center', transition: 'opacity 0.2s' }}>
                    En savoir plus →
                  </Link>
                </div>
              </div>
