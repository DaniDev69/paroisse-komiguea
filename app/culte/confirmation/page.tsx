...
type Confirmation = { id: number; nom: string; prenom: string; categorie: string; annee: number; pereCelebrant: string; photos: Photo[] }

const CandidatCard = ({ candidat }: { candidat: Confirmation }) => {
  ...
  return (
    ...
    >
      <div style={{ position: 'relative', height: '220px', background: 'linear-gradient(135deg, #7A1A1A 0%, #C9401A 100%)', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '18px 20px' }}>
        ...
        <span style={{ display: 'inline-block', marginTop: '8px', background: '#FFF5F0', color: '#7A1A1A', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>{candidat.categorie}</span>
      </div>
    </div>
  )
}
...
export default function ConfirmationPage() {
  const [confirmes, setConfirmes] = useState<Confirmation[]>([])
  ...
  useEffect(() => {
    fetch('/api/listes/confirmations')
      .then(r => r.json())
      .then(data => { setConfirmes(Array.isArray(data) ? data : []); setChargement(false) })
      .catch(() => setChargement(false))
  }, [])

  const visibles = voirTout ? confirmes : confirmes.slice(0, 6)

  const handleDownloadPhotos = async () => {
    if (!anneeDownload) return alert('Veuillez sélectionner une année')
    const filtered = confirmes.filter(c => c.annee === parseInt(anneeDownload))
    if (filtered.length === 0) return alert('Aucun confirmé pour cette année')
    ...
    for (let i = 0; i < photos.length; i++) {
      ...
      link.href = photos[i]
      link.download = `confirmation_${anneeDownload}_${i + 1}.jpg`
      link.click()
  ...
  return (
    <>
      <main>
        <section style={{ background: 'linear-gradient(160deg, #7A1A1A 0%, #C9401A 100%)', padding: '80px 40px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <p style={{ fontFamily: 'Crimson Text, serif', fontSize: '22px', color: '#FBF4E0', fontStyle: 'italic', maxWidth: '650px', margin: '0 auto 32px' }}>
              Vous êtes sur votre espace dédié au sacrement de la <strong style={{ color: '#FFD700' }}>Confirmation</strong>
            </p>
            <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', flexWrap: 'wrap' }}>
              <Link href="/culte/confirmation/liste" style={{ display: 'inline-block', background: '#FFD700', color: '#7A1A1A', fontFamily: 'Lato, sans-serif', fontSize: '12px', fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', padding: '14px 32px', textDecoration: 'none', borderRadius: '4px' }}>
                Consulter la Liste
              </Link>
            ...
            {showDownload && (
              <div style={{ maxWidth: '400px', margin: '24px auto 0', background: 'rgba(255,255,255,0.1)', borderRadius: '10px', padding: '24px', border: '1px solid rgba(255,255,255,0.2)' }}>
                <p style={{ fontFamily: 'Cinzel, serif', fontSize: '12px', letterSpacing: '1px', color: '#FFD700', marginBottom: '12px', textTransform: 'uppercase' }}>Choisir une Année</p>
                ...
                <button onClick={handleDownloadPhotos} style={{ width: '100%', padding: '12px', background: '#FFD700', color: '#7A1A1A', border: 'none', borderRadius: '6px', fontFamily: 'Lato, sans-serif', fontSize: '12px', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', cursor: 'pointer' }}>
                  ⬇ Télécharger
                </button>
              </div>
            )}
            <div style={{ width: '60px', height: '1px', background: '#FFD700', margin: '32px auto 0', opacity: 0.5 }}></div>
          </div>
        </section>
        ...
        <section style={{ background: '#FFFFFF', padding: '60px 40px' }}>
          <div style={{ maxWidth: '800px', margin: '0 auto', textAlign: 'center' }}>
            ...
            <h2 style={{ fontFamily: 'Cinzel, serif', fontSize: '30px', fontWeight: 400, color: '#0D2B55', marginBottom: '32px' }}>La Confirmation, Don de l&apos;Esprit</h2>
            ...
            <p style={{ fontFamily: 'Crimson Text, serif', fontSize: '19px', color: '#4A5568', lineHeight: 1.85 }}>
              La Confirmation est le sacrement par lequel le baptisé reçoit le don plénier de l&apos;Esprit Saint. Elle fortifie et confirme la grâce du Baptême.
            </p>
          </div>
        </section>
        ...
        <section style={{ background: '#FAF8F3', padding: '60px 40px' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '48px' }}>
            ...
            <h2 style={{ fontFamily: 'Cinzel, serif', fontSize: '30px', fontWeight: 400, color: '#0D2B55' }}>Nos Confirmés</h2>
          ...
          {chargement ? (
            ...
            <div style={{ textAlign: 'center', padding: '40px' }}><p style={{ fontFamily: 'Crimson Text, serif', fontSize: '18px', color: '#6B7A8D', fontStyle: 'italic' }}>Aucun confirmé enregistré pour le moment.</p></div>
          ) : (
            <>
              ...
              {confirmes.length > 6 && (
                <div style={{ textAlign: 'center', marginTop: '40px' }}>
                  <button onClick={() => setVoirTout(!voirTout)} style={{ background: '#7A1A1A', color: '#FBF4E0', border: 'none', borderRadius: '6px', padding: '14px 36px', fontFamily: 'Lato, sans-serif', fontSize: '12px', fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', cursor: 'pointer' }}>
                    {voirTout ? '▲ Voir Moins' : `▼ Voir Plus (${confirmes.length - 6} restants)`}
                  </button>
                </div>
              )}
            </>
