...
export default function ConfirmationListePage() {
...
const handleDownload = async () => {
  ...
  try {
    const res = await fetch(`/api/listes/confirmés?annee=${annee}&categorie=${encodeURIComponent(categorie)}`)
    ...
    doc.setFontSize(13)
    doc.text(`Liste des confirmés — ${categorie} — ${annee}`, 14, 32)
    doc.setFontSize(10)
    doc.text('Nom', 14, 50)
    ...
    doc.save(`confirmation_${annee}_${categorie}.pdf`)
    ...
    link.href = url
    link.download = `confirmation_${annee}_${categorie}.csv`
    link.click()
      ...
      <main>
        <section style={{ background: 'linear-gradient(160deg, #7A1A1A 0%, #C9401A 100%)', padding: '70px 40px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            <p style={{ fontFamily: 'Cinzel, serif', fontSize: '11px', letterSpacing: '3px', textTransform: 'uppercase', color: '#FBF4E0', marginBottom: '16px' }}>Registre</p>
            <h1 style={{ fontFamily: 'Crimson Text, serif', fontSize: '44px', fontStyle: 'italic', color: '#FFFFFF', lineHeight: 1.2 }}>Liste des Confirmés</h1>
          </div>
        </section>
        ...
        <section style={{ background: '#FAF8F3', padding: '80px 40px' }}>
          <style>{`
            ...
            .download-btn { width: 100%; padding: 16px; background: #7A1A1A; color: #FFFFFF; border: none; border-radius: 6px; font-family: Lato, sans-serif; font-size: 12px; font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; cursor: pointer; transition: background 0.2s, opacity 0.2s; margin-top: 8px; }
            .download-btn:hover { background: #C9401A; }
          ...
          <div className="liste-form">
            <div className="form-group">
              <label className="form-label">Année de Confirmation</label>
            ...
            <div style={{ background: '#FFF5F0', borderRadius: '8px', padding: '16px 20px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '12px' }}>
              <span style={{ fontSize: '20px' }}>ℹ️</span>
              <p style={{ fontFamily: 'Crimson Text, serif', fontSize: '15px', color: '#7A1A1A', fontStyle: 'italic', margin: 0 }}>
                La liste contiendra : Nom, Prénom, Année de confirmation et Père célébrant
              </p>
            </div>
        ...
        <section style={{ background: '#FFFFFF', padding: '40px', textAlign: 'center' }}>
          <Link href="/culte/confirmation" style={{ display: 'inline-block', background: '#0D2B55', color: '#FBF4E0', fontFamily: 'Lato, sans-serif', fontSize: '11px', fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', padding: '14px 32px', textDecoration: 'none', borderRadius: '3px' }}>
            ← Retour Confirmation
          </Link>
        </section>
      </main>
