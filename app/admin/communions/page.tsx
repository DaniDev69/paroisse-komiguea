...
import { Communion } from '@prisma/client'
...
export default function AdminCommunion() {
  const { token, isConnected } = useAuth()
  const router = useRouter()
  const [liste, setListe] = useState<Communion[]>([])
  ...
  const charger = async () => {
    setChargement(true)
    try {
      const res = await fetch('/api/listes/communions', { headers: { Authorization: `Bearer ${token}` } })
  ...
  const handleSupprimer = async (id: number) => {
    ...
    try {
      await fetch('/api/listes/communions', {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
        ...
        <div style={{ borderRadius: '12px', overflow: 'hidden', boxShadow: '0 4px 24px rgba(13,43,85,0.08)' }}>
          <h2 style={{ fontFamily: 'Cinzel, serif', fontSize: '18px', fontWeight: 400, color: '#0D2B55', padding: '20px 24px', background: '#FFFFFF', borderBottom: '1px solid rgba(201,168,76,0.2)', margin: 0 }}>
            📋 Liste des communiés ({liste.length})
          </h2>
          {chargement ? (
